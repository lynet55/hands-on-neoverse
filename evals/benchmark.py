...
class BenchmarkEvaluator:
    ...
    def _step(
        ...
        if self.run_render:
            ...
            target_rgb, _, _, _ = self.reconstructor.gs_renderer.ras/terizer.forward(
                gaussians,
...
def parse_args():
    ...
    p.add_argument("--mode", type=str, default="interpolation",
                   choices=["interpolation", "reconstruction", "both"],
                   help="interpolation: render held-out in-between frames via velocity "
                        "(default; measures the velocity field). reconstruction: the static "
                        "use_motion=False keyframe re-render + segmentation metrics. both: run each.")
...
def load_reconstructor(args):
    ...
    if _is_wrapped_training_ckpt(args.reconstruction_model_path):
        ...
        ckpt = torch.load(args.reconstruction_model_path, map_location="cpu", weights_only=False)
        ...
        mm.load_model(args.reconstruction_model_path, device=args.device, torch_dtype=torch.bfloat16)
        reconstructor: WorldMirror = mm.fetch_model("reconstructor")
        if reconstructor is None:
            raise RuntimeError(
                f"ModelManager could not load a reconstructor from {args.reconstruction_model_path}."
            )
